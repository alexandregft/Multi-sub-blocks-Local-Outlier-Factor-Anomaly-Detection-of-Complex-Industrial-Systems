...
def generate_result_validation_lof(scaler_list,list_corsets,Validation_df,cols_feature,list_features_group,n_neighbors,contamination):
...
def test_different_fault_lof(scaler_list,list_corsets,Testing_df,Validation_df,cols_feature,list_group_features,n_neighbors,contamination):
   ...
   the observation j is an outlier.
    ...
    list_tab_res_fault = []
    detected_issues = []
    Tab_all_results = np.zeros((Testing_df['faultNumber'].nunique(), len(list_group_features), n_simu, len_sample))
    list_lof_classifier = []
    ...
    for i, group_features in enumerate(list_group_features):
        ...
        x_val=Validation_df[cols_feature].to_numpy()
        x_val_transform=x_val[:,list_group_features[i]]
        x_val_transform=scaler.transform(x_val_transform)
        y_pred=clf.predict(x_val_transform)
        y_score=-clf.score_samples(x_val_transform)
        ...
        list_treshold.append(threshold)
        

    for fault_num in range(1, Testing_df['faultNumber'].nunique()+1):
